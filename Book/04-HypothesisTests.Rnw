...
gf_histogram(~ prop, binwidth = 0.04, data = Randomization.Match )
@
\authNote{need to make vertical line at .88}
...
@
\authNote{need to label each bar, in lattice 'label = TRUE'}
...
gf_histogram(~ cor, v = c(0.43), binwidth = 0.05, data = Randomization.Mal)
@
\authNote{need to make line at .43}
...
gf_dotplot(~ BodyTemp, v = c(98.26), binwidth = .1, stackratio = 2, dotsize = 1, 
            data = BodyTemp50)
@
\authNote{v argument is here but there is no line, dotPlot( ~ BodyTemp, v = c(98.26), width = .1, cex = .2, data = BodyTemp50)}
...
gf_histogram( ~ result, binwidth = .01, v = c(98.40, 98.6, 98.81), data = Randomization.Temp)
@
\authNote(need to make vertical line at v = c(98.40, 98.6, 98.81))
...
gf_histogram( ~ result, binwidth = .01, v = c(0.34, -0.34), data = Randomization.Temp2)
@
\authNote{need to make vertical line at v = c(0.34, -0.34)}
\end{enumerate}
...
@
\authNote{need to make vertical lines, in lattice(v = c(98.26, 98.6)}
...
gf_histogram( ~ result, binwidth = .01, v = c(98.26, 98.4), 
          fill = ~(98.19 <= result & result <= 98.62), 
          xlim = c(97.8, 99.0), data = Randomization.Temp3, show.legend = FALSE) # randomization
gf_histogram( ~ mean, binwidth = .01, v = c(98.26, 98.4), 
          fill = ~(98.05 <= mean & mean <= 98.46), 
          xlim = c(97.8, 99.0), data = Boot.Temp, show.legend = FALSE) # bootstrap
@
\authNote{need to make vertical lines, in lattice at v = c(98.26, 98.4), then for plot 2, v = c(98.26, 98.4)}
\end{enumerate}
...
head(Randomization.Temp,3)
gf_histogram( ~ diffmean, data = Randomization.Temp, binwidth = .12, v = c(0.176, -0.176))
@
\authNote{need to make vertical lines, v = c(0.176, -0.176)}
<<>>=
...
head(Randomization.Temp2, 3)
gf_histogram( ~ diffmean, data = Randomization.Temp2, binwidth = .15, v = c(0.176, -0.176))
@
\authNote{need to make vertical line, at v = c(0.176, -0.176)}
<<>>=
...
head(Randomization.Temp3, 3)
gf_histogram( ~ diffmean, data = Randomization.Temp3, binwidth = .12, v = c(0.176, -0.176))
@
\authNote{need to make vertical lines, at v = c(0.176, -0.176)}
<<>>=
