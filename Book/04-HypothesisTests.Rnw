...
gf_dotplot( ~ No.Caffeine, binwidth = 0.1, dotsize = .6, fill = ~(No.Caffeine >=3.5), 
...
gf_dotplot(~ diffmean, binwidth = 0.03, dotsize = .7, fill = ~(diffmean >= 0.79), 
...
prop( ~ (diffmean <= -0.79 | diffmean >= 0.79), data = Randomization.Smiles)
2 * prop( ~ diffmean >= 0.79, data = Randomization.Smiles )
gf_dotplot( ~ diffmean, binwidth = .03, dotsize = .7, fill = ~(diffmean >= 0.79), 
...
@
\authNote{v argument is in original plot but there is no line, dotPlot( ~ BodyTemp, v = c(98.26), width = .1, cex = .2, data = BodyTemp50)}
