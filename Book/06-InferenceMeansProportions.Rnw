...
head(Wetsuits, 3)
gf_dotplot(~ Wetsuit, xlim = c(1.1, 1.8), binwidth = .04, stackratio = 2, data = Wetsuits) # to check for normality
gf_dotplot(~ NoWetsuit, xlim = c(1.1, 1.8), binwidth = .04, stackratio = 2, data = Wetsuits) # to check for normality
@
...
gf_dotplot(~(Wetsuit - NoWetsuit), binwidth = .01, dotsize = .5, data = Wetsuits)
@
...
gf_dhistogram( ~ prop, data = Rand.Flowers, binwidth = 1/27)
...
gf_dhistogram(~prop, data = Rand.toys, binwidth=1/16)
@
