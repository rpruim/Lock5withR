...
gf_point( Ants ~ Filling, data = SandwichAnts) %>% 
  gf_line(Ants ~ Filling, data = SandwichAnts)
...
<<Figure8.3, cache = TRUE, opts.label = "fig4", message = FALSE, warning = FALSE>>=
...
<<Figure8.4, fig.keep = 'last', message = FALSE, warning = FALSE>>=
...
@
\authNote{not sure if this need to be converted into ggformula}
...
gf_dhistogram( ~ F, bins = 12, data = small.rand) %>%
  gf_vline(xintercept = 10.27)
@
...
gf_point( shift ~ treatment, data = JetLagKnees, type = c('p', 'a') )
gf_boxplot( treatment ~ shift, data = JetLagKnees )
@
\authNote{JetLagKnees data source not found}
<<>>=
