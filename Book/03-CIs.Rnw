...
@
\authNote{density instead of count}
...
p - MoE
p + MoE
gf_dotplot(~ prop, binwidth = .002, colour = ~(0.215 <= prop & prop <= 0.335), data = Sampledist.deg, show.legend = FALSE)
@
...
\authNote{do I have to convert these to ggformula if they're not in lattice?}
...
@
Notice the \argument{colour=} for marking the confidence interval.
...
stat + 2 * SE
@
\authNote{original plot used variable 'M' but it was unrecognized. I assumed they wanted to filter 'diffmean', also in the book this is by count, not density}
