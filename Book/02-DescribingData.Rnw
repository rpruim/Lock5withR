...
The various options available for the \function{gf_histogram()} function enable us to replicate Figure 2.8, some including centering, adding counts, labels, and limit to the y-axis (similar for x-axis). Using \function{gf_histogram()} measures the y-axis by density rather than count. This is useful for determing relative amounts.
...
%>% gf_lims(x = c(35, 80))
gf_dotplot( ~ SanFrancisco, binwidth = 1, dotsize = .5, stackratio = 2, data = April14Temps)
%>% gf_lims(x = c(45, 65))
@
...
<<Example2.26, opts.label = "fig1">>=
favstats( ~ Longevity, data = MammalLongevity)
gf_boxplot(Longevity ~ 'NULL', xlab = '', data = MammalLongevity)
@
\authNote{I think there is probably a better way to plot this against nothing than using 'NULL' but I'm not sure how, also not sure how I can change the orientation of this plot}
