...
A way to look at multiple groups simultaneously is by using \emph{comparative plots} such as a \emph{segmented bar chart} or \emph{side-by-side bar chart}. We use the \argument{fill} argument for this. What \argument{fill} does depends a bit on the type of graph. Using \argument{fill} with \function{gf_histogram()} doesn't work so well because it is difficult to overlay histograms.%
...
%I think this explanatory paragraph and footnote need to be rewritten to explain fill =, gf_histogram(), and position

Notice the addition of \argument{fill=} (to group), \argument{position=} (to segment the graph), and \argument{auto.key=TRUE} (to build a simple legend so we can tell which groups are which).
<<Figure2.2>>=
gf_bar( ~ Award, fill = ~Gender, auto.key = TRUE, data = StudentSurvey)
@

<<Figure2.2b>>=
gf_bar( ~ Gender, fill = ~Award, position = 'dodge', auto.key = TRUE, data = StudentSurvey)
@
...
We can control the (approximate) number of bins using the \option{nint} argument, which may be abbreviated as \option{n}. The number of bins (and to a lesser extent the positions of the bins) can make a histogram look quite different.
...
The various options available for the \function{histogram()} function enable us to replicate Figure 2.8, some including centering, adding counts, labels, and limit to the y-axis (similar for x-axis).
