...
<<Figure2.8, opts.label = "fig3", tidy = FALSE>>=
...
@
\authNote{histogram( ~ Exercise, type = "count", width = 2, center = 2, 
          right = FALSE, ylim = c(0, 70), data = StudentSurvey) was the original 2nd graph. The           replacement looks the same but I get the Warning message 'Removed 1 rows containing             non-finite values (stat_bin)'}
...
<<freqpolygon-illustrated, echo = FALSE>>=
...
\R\ also provides a ``smooth'' version called a density plot; just change the function name from \function{histogram()} to \function{densityplot()}.
<<densityplot>>=
