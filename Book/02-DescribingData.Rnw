...
@
\authNote{y axis is count but it should be density}
...
\R\ also provides a ``smooth'' version called a density plot; just change the function name from \function{gf_histogram()} to \function{gf_density()}, also \function{gf_dens()} is a modified version of \function{gf_density} that isn't shaded in and may be useful for layering plots.
