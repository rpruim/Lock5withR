...
@
\iffalse
In this case, the file is also distributed with the \pkg{Lock5withR} package, so
...
From the dataset we named as \dataframe{OneTrueLove}, we can use the \function{prop()} function to quickly find \term{proportions}.
<<proportion>>=
prop( ~ Response, data = OneTrueLove)
@

\subsubsection*{Table 2.1}

We can also tabulate the categorical variable to display the \emph{frequency} by using the \function{tally()} function. The default in tallying is to not include the row totals, or column totals when there are two variables. These are called marginal totals and if you want them, you can change the default.
<<Table2.1>>=
...
gf_bar( ~ Response, data = OneTrueLove, horizontal = TRUE)
@
...
\subsubsection*{Table 2.3}

<<Table2.3>>=
tally( ~ Response + Gender, data = OneTrueLove)
@

\subsubsection*{Table 2.4}
...
\subsubsection*{Figure 2.2}

A way to look at multiple groups simultaneously is by using \emph{comparative plots} such as a \emph{segmented bar chart} or \emph{side-by-side bar chart}. We often use the \argument{fill} argument for this. \argument{fill} is used when the assigned data is represented as an area, rather than a line or point.
\footnote{For coloring a line or point, \function{colour} is used}

Notice the addition of \argument{fill=} (to group) and \argument{position=} (to segment the graph). The default of \argument{position=} is to stack the bar graph, so for the first example we can disclude the argument.
\authNote{Rewrote paragraph and footnote}
<<Figure2.2>>=
...
\begin{itemize}
  \item
		...
		\function{gf_density()},\authNote{not sure if it should be gf_density(), gf_dhistogram() or gf_dens()} \function{gf_boxplot()}, etc.)
	\item
...
<<Figure2.6, opts.label = "fig1">>=
gf_dotplot(~ Longevity, binwidth = 1, dotsize = .75, data = MammalLongevity)
@
\authNote{This is showing the density but the original showed number of cases, I'm not sure how to change this}
\subsubsection*{Table 2.15}
...
\subsubsection*{Figure 2.7}

Histograms are a way of displaying the distribution of a quantitative variable.
<<Figure2.7>>=
gf_histogram( ~ Longevity, data = MammalLongevity)
gf_histogram( ~ Longevity, binwidth = 5, data = MammalLongevity)
@

We can control the (approximate) number of bins using the \option{bins} argument. The number of bins (and to a lesser extent the positions of the bins) can make a histogram look quite different.
<<Figure2.7b, opts.label = "fig3">>=
...
\subsubsection*{Figure 2.8}

The various options available for the \function{gf_histogram()} function enable us to replicate Figure 2.8, some including centering, adding counts, labels, and limit to the y-axis (similar for x-axis). Using \function{gf_dhistogram()} measures the y-axis by density rather than count. This is useful for determing relative amounts.
<<Figure2.8, opts.label = "fig3", tidy = FALSE, message = FALSE, warning = FALSE>>=
...
The only thing that changes is \function{gf_histogram()} becomes \function{gf_freqpoly()}.

<<freqpolygon, message = FALSE, warning = FALSE>>=
gf_freqpoly( ~ Exercise, bins = 10, data = StudentSurvey)
@
\authNote{'removed 1 rows containing non-finite values (stat_bin)' error and y axis is count instead of density}
...
<<freqpolygon-illustrated, echo = FALSE, message = FALSE, warning = FALSE>>=
gf_dhistogram( ~ Exercise, bins = 10, data = StudentSurvey) %>% gf_freqpoly( ~ Exercise, bins = 10, data = StudentSurvey)
@

\R\ also provides a ``smooth'' version called a density plot; just change the function name from \function{gf_histogram()} to \function{gf_density()}.
<<densityplot, message = FALSE, warning = FALSE>>=
gf_density( ~ Longevity, data = MammalLongevity)
gf_density( ~ BirthRate, data = AllCountries )
@
...
Note that the example asks about subsets of \dataframe{ICUAdmissions}--specifically about 20-year-old and 55-year-old patients. In this case, we can manipulate the data (to name a new data set) with the subset command. Here are some examples.
\begin{enumerate}
  \item 
...
\subsubsection*{Figure 2.18}

The \argument{dotsize} argument controls ``character expansion" and can be used to make the plotting ``characters" larger or smaller by specifying the scaling ratio. \argument{stackratio} allows you to adjust the vertical distance between points. Chaining \function{gf_lims()} lets you set the limits for the x-axis.
<<Figure2.18, opts.label = "fig1">>=
...
@
\authNote{add explanation for arguments and chaining gf_lims()}
...
To see that the distribution is indeed symmetric and approximately bell-shaped, you can chain the function \function{gf_dens()}. It can be helpful to set alpha (the transparency argument) lower than 1 to make the plot easier to read.
<<Example2.17>>=
...
\subsubsection*{Figure 2.21}

<<Figure2.21>>=
gf_histogram( ~ Close, binwidth = 20, data = SandP500)
@

\subsubsection*{Example 2.20}

The text uses a histogram to estimate the \term{percentile} of the daily closing price for the 
S\&P 500 but we can also find the exact percentiles using the \function{quantile()} function. 
<<Example2.20>>=
...
@
\authNote{I think there is probably a better way to plot this against nothing than using '' but I'm not sure how, also not sure how I can change the orientation of this plot}

<<Figure2.31, include = FALSE>>=
<<Example2.26>>
@

\subsubsection*{Figure 2.32}

<<Figure2.32>>=
gf_boxplot(Smokers ~ '', xlab = '', data = USStates)
@
\authNote{this would be better with a horizontal orientation}
...
\subsubsection*{Figure 2.33}

<<Figure2.33, message = FALSE, warning = FALSE>>=
...
\subsection*{One Quantitative and One Categorical Variable}
...
The most common way to look at two quantitative variables is with a scatterplot.  The \pkg{ggformula} function for this is \function{gf_point()}, and the basic syntax is
<<eval = FALSE>>=
...
\subsubsection*{Table 2.31}

<<Table2.31>>=
CricketChirps
@

\subsubsection*{Figure 2.50}
...
\subsubsection*{Figure 2.63}

<<Figure2.63>>=
gf_point(Tip ~ Bill, size = .5, data = RestaurantTips)
@

\subsubsection*{Example 2.39}

When the relationship between variables is sufficiently \emph{linear}, you may be able to predict the value of a variable using the other variable. This is possible by fitting a \emph{regression line}. To plot this in \R, all we need to do is chain \function{gf_lim()} to the scatter plot and give it the corresponding arguments.
<<Example2.39>>=
...
The \pkg{ggformula} graphics plots make it particularly easy to generate plots that 
...
the \argument{fill} or \argument{colour} arguments).  These plots can reveal the 
...
When unequal bin sizes are specified, you should use \function{gf_dhistogram()}
...
It would be incorrect to choose \function{gf_histogram()} since
...
\subsection*{show.legend}

\option{show.legend = TRUE} turns on a simple legend for the grouping variable.  
...
\argument{dotsize} for \function{gf_dotplot()} and \argument{size} for \function{gf_point()} arguments control ``character expansion" and can be used to make
...
\subsection*{gf_labs(), xlab, ylab, title, subtitle}

You can add a title or subtitle, or change the default labels of the axes in one of two ways. \function{gf_labs()} allows the chaining syntax to be used.
...
\authNote{does this only pertain to lattice plots?}

\subsubsection*{size, theme, color, shape}
...
\authNote{this trellis stuff for lattice seems irrelevant now}
