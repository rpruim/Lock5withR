...
A way to look at multiple groups simultaneously is by using \emph{comparative plots} such as a \emph{segmented bar chart} or \emph{side-by-side bar chart}. We use the \argument{groups} argument for this. What \argument{groups} does depends a bit on the type of graph. Using \argument{groups} with \function{gf_histogram()} doesn't work so well because it is difficult to overlay histograms.%
