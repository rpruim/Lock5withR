...
gf_freqpoly( ~ Exercise, binwidth = 5, data = StudentSurvey)
@
...
The \argument{dotsize} argument controls ``character expansion" and can be used to make the plotting ``characters" larger or smaller by specifying the scaling ratio. \argument{xlim} sets the limits for the x-axis. \argument{stackratio} allows you to adjust the vertical distance between points. Chaining \function{gf_lims()} lets you set the limits for the x-axis.
...
The most common way to look at two quantitative variables is with a scatterplot.  The \pkg{lattice} function for this is \function{xyplot()}, and the basic syntax is
<<eval = FALSE>>=
