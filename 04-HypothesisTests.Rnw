...
\begin{enumerate}
  \item
    $H_0$: $\rho = 0$;  $H_a$: $\rho > 0$
	\item
		Test statistic:  $r$ (the sample correlation)
	\item
		We simulate a world in which $\rho = 0$:
<<>>=
Randomization.Mal <- do (10000) * cor(NFL_Malevolence~ shuffle(ZPenYds), data=MalevolentUniformsNFL)
...
\begin{enumerate}
  \item
    $H_0$: $\mu = 98.6$;  $H_a$: $\mu \neq 98.6$
  \item
		Test statistic:  $\bar x$ (the sample mean)
...
histogram(~result, width=.025, v=c(98.26, 98.94), data=Randomization.Temp)
@
...
<<>>=
Randomization.Temp2 <- do(1000) * ( mean( ~BodyTemp, data=resample(BodyTemp50) ) - 98.26 ) 
head(Randomization.Temp2, 3)
mean( ~ result, data=Randomization.Temp2 )
histogram(~result, width=.025, v=c(0.34, -0.34), data=Randomization.Temp2)
@
From this we can estimate the p-value:
<<>>=
