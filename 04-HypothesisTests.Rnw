...
@
Note the addition of ` 's around the resulting name for the difference in means, No Caffeine. We do this because there is a space in the name.
<<Example4.18c>>=
...
\begin{enumerate}
  \item
    $H_0$: $p = 0.5$;  $H_a$: $p > 0.5$
	\item
		Test statistic:  $\hat p = 0.5$ (the sample proportion) %check book again
	\item
		We simulate a world in which $p = 0.5$:

<<Example4.19a, seed=1234, cache=TRUE>>=
...
\begin{enumerate}
  \item
    $H_0$: $p = 0.5$;  $H_a$: $p \neq 0.5$
  \item
		Test statistic:  $\hat p = 0.8$ (the sample proportion)
	\item
		We simulate a world in which $p = 0.5$:
<<Example4.20a>>=
prop( ~ (prop >= 0.8), data=RandomizationDist )
prop( ~ (prop <= 0.2), data=RandomizationDist )
histogram(~prop, label=TRUE, type="count", groups=(prop <= 0.2 | prop >= 0.8), data=RandomizationDist)
@
...
<<Section4.5, seed=1234, cache=TRUE>>=
...
<<Example4.35, seed=1234, cache=TRUE>>=
