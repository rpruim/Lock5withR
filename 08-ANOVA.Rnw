...
histogram( ~ F, width=.75, center=.25, data=rand)
plotDist("f", df1=2, df2=21, add=TRUE)
@
%histogram of randomization with 2 and 21 df?
...
t.test(~Ants, data=PB)
@
%how to do correct t-test with df=21

\subsubsection*{Example 8.8}

<<Example8.8>>=
noPB <- subset(SandwichAnts, Filling != "Peanut Butter")
t.test(Ants ~ Filling, data=noPB)
@
%wrong
