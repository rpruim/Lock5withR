...
%INCLUDE REAL INTRODUCTION TO CHAPTER 1
...
\subsubsection*{A Student Survey}
...
sets are built into \R\ and its packages. The package for our text is \pkg{Lock5Data} which comes with a number of data sets. %1.4 p.13
...
\subsubsection*{Data on Countries}

<<>>=
head(AllCountries)
summary(AllCountries)
@
%Info just on Iceland--observation 86
...
%ex 1.15 select a random sample of 5 of AllCountries


\section{Experiments and Observational Studies}

\subsection*{Types of Statistical Studies}
...
%random selection and randomized experiment
%matched pairs?

\subsubsection*{Vehicles and Life Expectancy}
...
information for the \dataframe{AllCountries} data set by typing
...
good idea to include an extra row at the top which names the variables. Take a look at
Chapter 0 to learn how to get the data from Excel into \R.
