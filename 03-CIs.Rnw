...
<<bootstrap-samplesize, opts.label="fig3">>=
...
<<message=FALSE, seed=1234, opts.label="fig1">>=
...
<<seed=1234>>=
Boot.L1 <- do(1000) * mean( ~NumLaughs, data=resample(Laughter))
mean(~result, data = Boot.L1)
Boot.L2 <- do(1000) * mean( ~NumLaughs, data=resample(Laughter))
mean(~result, data = Boot.L2)
Boot.L3 <- do(1000) * mean( ~NumLaughs, data=resample(Laughter))
mean(~result, data = Boot.L3)
@
