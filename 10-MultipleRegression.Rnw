...
ink <- makeFun(lm(Price ~ PPM + CostBW, data = InkjetPrinters))
ink(PPM = 3.0, CostBW = 3.7)
@
...
<<Example10.6>>=
anova(lm(Price ~ PPM, data = InkjetPrinters))
anova(lm(Price ~ PPM + CostBW, data = InkjetPrinters))
@
%anova for multiple regression model (F-stat)

\subsubsection*{Example 10.7}

<<Example10.7>>=
summary(lm(Price ~ PhotoTime + CostColor, data = InkjetPrinters))
anova(lm(Price ~ PhotoTime + CostColor, data = InkjetPrinters))
@
...
xyplot(resid(body.model) ~ fitted(body.model), type=c("p", "r"))
@
