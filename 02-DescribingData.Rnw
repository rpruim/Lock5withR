...
<<Figure2.18, opts.label="fig1", echo=FALSE>>=
dotPlot(~ DesMoines, width=1, cex=.05, xlim=c(35, 80), data=April14Temps)
dotPlot(~ SanFrancisco, width=1, cex=.05, xlim=c(35, 80), data=April14Temps)
@
...
dotPlot(~TV|Gender, layout=c(1,2), width=1, cex=.25, data = StudentSurvey)
@
...
Resid.c <- 10.00 - 4.02
Resid.c
# resid(lm(Tip~Bill, data=RestaurantTips))
@
