...
The data set for the One True Love survey is not readily available in the package for our textbook but the authors do provide us with the necessary information to own data spreadsheet (in either Excel or Google) and import it into \R. (See Chapter 0 for instructions.) %One could also use any other data set as an example.

From the dataset we named as \dataframe{TrueLove}, we can display the \emph{frequency} of the variable by utilizing \function{tally()}.
<<tally-1way>>=
...
<<bargraph>>=
bargraph(~ Response, data=TrueLove)
bargraph(~ Response, data=TrueLove, horizontal=TRUE )
@
...
<<tally-2way>>=
tally(~ Response + Gender, data = TrueLove)
tally(Response ~ Gender, data = TrueLove)
tally(~ Response | Gender, data = TrueLove)
@
...
A way to look at multiple groups simultaneously is by using \emph{comparative plots} such as a \emph{segmented bar chart} or \emph{side-by-side bar chart}.
...
We use \argument{auto.key=TRUE} to build 
...
\subsubsection*{Histograms (and density plots) for quantitative variables}
...
We can illustrate example 2.9 
<<fig.width=3,out.width='.3\\textwidth'>>=
histogram(~ Pulse, data=StudentSurvey, width=5)
histogram(~ Exercise, type="count", data=StudentSurvey, width=2) 
histogram(~ Piercings, data=StudentSurvey, width=1)
@
%fix exercise histogram
...
Recall that a statistic is a number computed from data.  
Numerical summaries are computed using the same template as graphical summaries.
The \term{mean} and the \term{median} are key summaries which describe the center of a distribution.
%explain the subset? or refer to Chapter 0?
%example 2.11
<<center>>=
