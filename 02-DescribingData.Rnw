...
@
%explain the differences in packages?
...
We have covered many different ways to obtain the \term{five number summary} but \function{summary()} is the fastest way to obtain the summaries of the quantitative variables in a data set and \function{favstats()} is the fastest way to obtain more detailed statistics about a specific quantitative variable.
<<five-number-summary>>=
