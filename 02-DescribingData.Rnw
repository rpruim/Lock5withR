...
From the dataset we named as \dataframe{OneTrueLove}, we can tabulate the categorical variable to display the \emph{frequency} by using the \function{tally()} function. The default in tallying is to not include the row totals, or column totals when there are two variables. These are called marginal totals and if you want them, you can change the default.
<<tally-1way>>=
...
<<histogram2, opts.label="fig3", warning=FALSE>>=
...
<<xhistogram, opts.label="fig3", warning=FALSE>>=
...
<<Example2.9, opts.label="fig3", tidy=FALSE>>=
...
<<dotplot-example, opts.label="fig1", echo=FALSE>>=
...
%how to include z-scores with R? using scale?
