...
@
Notice the "less" for the alternative hypothesis.

\subsubsection*{Example 6.8}
%book and Pruim approximation method different
...
<<Figure6.6,cache=TRUE,opts.label="fig3">>=
...
@
%how to fill in tails
<<Example6.12c,fig.keep='last'>>=
plotFun(dt(x, df=15) ~x, x.lim=c(-4, 4))
plotDist("t", params=list(df=15), type="h", groups= x > 2.131)
ladd(grid.text("you can add text like this",2.1,.1, default.units="native", hjust=0))
@
...
<<Example6.13a>>=
head(Flight179, 3)
dotPlot(~Flight179, cex = 0.25, data = Flight179) # to check distribution
@
...
densityplot(~Time, data = CommuteAtlanta) # to check distribution
@
<<Example6.14b>>=
...
dotPlot(~Rent, width=200, cex=.25,  data=ManhattanApartments) # to check distribution
@
...
@
%how to graph t-distribution like xpnorm?
...
head(Boot.Love, 3)
histogram(~Agree.Male, data=Boot.Love)
plotFun(dnorm(x, 0.050, 0.0176)~x, add=TRUE) # add normal distribution from Example 6.20
@
...
%z.star is a little different from book
<<Example6.21>>=
success <- c(158, 109)
n <- c(444,922)
prop.test(success, n, conf.level=0.90, correct=FALSE)
@
%make note of creating data, correct=FALSE?
...
prop.test(decision~agegroup, correct=FALSE, data=SplitSteal)
@
...
<<Figure6.21, cache=TRUE, fig.keep='last'>>=
BootE <- do(1000) * diff(mean(Exercise~Gender, data=resample(ExerciseHours)))
head(BootE, 3)
histogram(~M, width=0.5, data=BootE)
plotFun(dnorm(x, 3.0, 2.39)~x, add=TRUE) # add normal distribution from Example 6.24
@

<<Figure6.21b, cache=TRUE, fig.keep='last'>>=
Random.Smiles <- do(1000) * diff(mean(Leniency ~ shuffle(Group), data = Smiles))
head(Random.Smiles, 3)
histogram(~smile, n=24, data=Random.Smiles)
plotFun(dnorm(x, 0, 0.39)~x, add=TRUE) # add normal distribution from Example 6.24
@
