...
p2 <- 109/922
diff <- (p1 - p2); diff
z.star <- qnorm(0.95); z.star
SE <- sqrt(p1*(1-p1)/444 + p2*(1-p2)/922)
diff - z.star * SE         # lower end of CI 
diff + z.star * SE         # upper end of CI
@
%Is there a test for the a difference in props without raw data?
...
histogram(~smile, data=Random.Smiles)
...
favstats(~Time, data=CommuteAtlanta)
@
%how to do test for difference in means for different datasets?
...
<<Example6.29>>=
t.test(Wetsuit, NoWetsuit, paired=TRUE, data=Wetsuits)
@
