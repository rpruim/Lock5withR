...
<<Figure6.2, cache=TRUE, opts.label="fig3">>=
P.05 <- do(2000) * rflip(50, .05)
dotPlot(~prop, data=P.05)

P.10 <- do(2000) * rflip(50, .10)
dotPlot(~prop, data=P.10)

P.25 <- do(2000) * rflip(50, .25)
dotPlot(~prop, data=P.25)

P.50 <- do(2000) * rflip(50, .50)
dotPlot(~prop, data=P.50)

P.90 <- do(2000) * rflip(50, .90)
dotPlot(~prop, data=P.90)

P.99 <- do(2000) * rflip(50, .99)
dotPlot(~prop, data=P.99)
@

\subsubsection*{Figure 6.3}

<<Figure6.3, cache=TRUE, opts.label="fig3">>=
n10 <- do(2000) * rflip(10, .10)
dotPlot(~prop, data=n10)

n25 <- do(2000) * rflip(25, .10)
dotPlot(~prop, data=n25)

n200 <- do(2000) * rflip(200, .10)
dotPlot(~prop, data=n200)
@
...
<<Figure6.9b,fig.keep='last'>>=
plotFun(dt(x, df=15) ~x, x.lim=c(-4, 4))
plotDist("t", params=list(df=15), type=c("h","l"), groups= x > 1.5, lty=1)
ladd(grid.text("1.5",1.5,.1, default.units="native", hjust=0))
@
...
<<Example6.13>>=
head(Flight179, 3)
dotPlot(~Flight179, cex = 0.25, data = Flight179) # to check for normality
@
...
dotPlot(~Rent, width=200, cex=.15,  data=ManhattanApartments) # to check for normality
@
...
ladd(grid.text("3.14",3,.025, default.units="native", hjust=0))
@
